...
public class PrePostProcessor {
    ...
    public final static int INPUT_WIDTH = 300;  //640
    public final static int INPUT_HEIGHT = 300;
    ...
    static ArrayList<Result> outputsToPredictions(int countResult, float[] outputs, float imgScaleX, float imgScaleY, float ivScaleX, float ivScaleY, float startX, float startY) {
        ...
        for (int i = 0; i< countResult; i++) {
            ...
            Result result = new Result((int)outputs[i* OUTPUT_COLUMN +5], outputs[i* OUTPUT_COLUMN +4], rect);  //class_index, score, rect
            results.add(result);
        }
        return results;
    }
